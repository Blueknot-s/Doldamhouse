        ...
        <div className="flex flex-wrap gap-4 mb-8">
          {[
            ...
            <button
              ...
              className={`px-8 py-3.5 rounded-sm text-xs font-bold uppercase tracking-widest transition-all shadow-sm ${
                activeTab === tab.id ? 'bg-doldam-dark text-white shadow-xl' : 'bg-white text-gray-400 hover:bg-gray-100'
              }`}
            >
              {tab.label}
            </button>
          ...
          {viewMode === 'form' && (
            <form onSubmit={handleSubmit} className="p-10 max-w-4xl mx-auto animate-fadeIn">
              ...
              <div className="space-y-3 mb-10">
                <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-2"><ImageIcon size={12}/> Images (이미지)</label>
                  ...
                  <div className="grid grid-cols-4 gap-4 mt-4">
                    {formData.images.map((img, index) => (
                      <div key={index} className="relative group">
                        <img src={img} alt={`Preview ${index}`} className="w-full h-32 object-cover rounded-sm border border-gray-200" />
                        <button 
                          type="button"
                        ...
                        {index === 0 && (
                          <span className="absolute bottom-1 left-1 bg-black text-white text-[10px] px-2 py-0.5 rounded-sm opacity-80">Main</span>
                        )}
                      </div>
                    ))}
                  </div>
